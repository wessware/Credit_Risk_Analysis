...
class KingaMetricRF:
    ...
    def train_model(self, X, y):
        """Train single RF model with fixed hyperparameters."""
        scale_pos_weight = (y == 0).sum() / (y == 1).sum()
        params = self.fixed_params.copy()
        #params['scale_pos_weight'] = scale_pos_weight
        self.rf_model = RandomForestClassifier(**params)
        self.rf_model.fit(X, y)
        print(f"Trained RF with scale_pos_weight: {scale_pos_weight:.2f}")

    def plot_top_features(self, save_path="visualizations/rf_top_features.png"):
        """Visualize top 20 features by importance."""
        if self.rf_model is None:
            raise ValueError("Model not trained.")
        importances = pd.Series(self.rf_model.feature_importances_, index=self.feature_names)
    ...
    def save(self, path="pickled_models/kingametric_rf_1.pkl"):
