...
class KingaMetricLGBM:
    def __init__(self):
        ...
        self.fixed_params = {
            'n_estimators': 777, 
            'num_leaves': 22, 
            'learning_rate': 0.010112001077434075, 
            'subsample': 0.7194769204588124, 
            'colsample_bytree': 0.9615946118649805, 
            'reg_alpha': 9.055848591044873, 
            'reg_lambda': 8.991135105776511}
    ...
    def train_model(self, X, y):
        """Train single LGBM model with fixed hyperparameters."""
        scale_pos_weight = (y == 0).sum() / (y == 1).sum()
        params = self.fixed_params.copy()
        #params['scale_pos_weight'] = scale_pos_weight
        self.lgb_model = LGBMClassifier(**params)
        self.lgb_model.fit(X, y)
        print(f"Trained LGBM with scale_pos_weight: {scale_pos_weight:.2f}")

    def plot_top_features(self, save_path="visualizations/lgbm_top_features.png"):
        """Visualize top 20 features by importance."""
        if self.lgb_model is None:
            raise ValueError("Model not trained.")
        importances = pd.Series(self.lgb_model.feature_importances_, index=self.feature_names)
    ...
    def evaluate(self, y_true, y_proba, y_pred):
    ...
    def train(self, filepath):
        ...
        print("Step 5: Training LGBM...")
        self.train_model(X_temp_sel, y_temp)

        print("Step 6: Predictions and evaluation...")
        test_proba = self.lgb_model.predict_proba(X_test_sel)[:, 1]
    ...
    def save(self, path="pickled_models/kingametric_lgbm_1.pkl"):
